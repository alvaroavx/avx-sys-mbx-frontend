<template>
  <v-app dark>
    ...
    <v-navigation-drawer v-model="drawer" disable-resize-watcher clipped app>
      <v-list>
        <v-list-item
    ...
    <v-app-bar
      ...
      :elevation="0"
      fixed
      style="pointer-events: none; border-bottom: 1px solid #f1f1f1"
      app
    >
      <v-toolbar-title>
        <nuxt-link :to="link">
      ...
      <v-spacer />
      <v-btn
        style="pointer-events: fill; margin-top: -12px"
        icon
        class="pr-6"
    ...
    >
      ...
      <v-list class="pt-12 mt-12 menu">
        <v-list-item>
          <!-- <v-list-item-title class="text-right pr-5" style="opacity: 0.8">
        ...
        <v-list-item to="/" @click="$fevu.logout()">
          <v-list-item-title class="text-right pr-5" style="opacity: 0.8">
...
<style>
.v-navigation-drawer.v-navigation-drawer--fixed.v-navigation-drawer--open.v-navigation-drawer--right.v-navigation-drawer--temporary.theme--dark {
  background-color: rgb(2, 189, 220) !important;
}
</style>
